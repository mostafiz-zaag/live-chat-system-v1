...
export class FAQSpecification {
    ...
    static matchSentence<T>(queryBuilder: SelectQueryBuilder<T>, sentence: string): SelectQueryBuilder<T> {
        if (sentence) {
    ...
    static matchSelf<T>(queryBuilder: SelectQueryBuilder<T>, userId: number): SelectQueryBuilder<T> {
        if (userId) {
    ...
    static matchAdminOrSelf<T>(queryBuilder: SelectQueryBuilder<T>, userId: number): SelectQueryBuilder<T> {
        if (userId) {
            return queryBuilder.andWhere('(createdBy.role = :adminRole OR createdBy.id = :userId)', {
